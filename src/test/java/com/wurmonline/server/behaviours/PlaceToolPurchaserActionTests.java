...
public class PlaceToolPurchaserActionTests extends ToolPurchaserTest {
    private static final String actionString = "Tool Purchaser";
    ...
    void testCorrectBehaviourReceived() {
        ...
        assertEquals("Tool Purchaser", entries.get(1).getActionString());
    }
    ...
    @Test
    void testPlaceToolPurchaserQuestionReceived() throws NoSuchFieldException, IllegalAccessException {
        assertTrue(menu.action(action, gm, wand, 0, 0, true,  0, 0, actionId, 0f));
        assertEquals(1, factory.getCommunicator(gm).getBml().length);
        new PlaceToolPurchaserQuestion(gm, Objects.requireNonNull(Zones.getTileOrNull(0, 0, true)), 0).sendQuestion();
